...
struct
{
    ...
    int y;
    char symbol;
} DrawPoint; // drawing point struct

char board[BOARD_HEIGHT][BOARD_WIDTH]; // drawing board array

// function to draw a point on the board
int draw(int x, int y, char symbol)
{
    y = BOARD_HEIGHT - y - 1;

    if (x < 0 || x >= BOARD_WIDTH || y < 0 || y >= BOARD_HEIGHT) // Check if the coordinates are within bounds
    {
        return -1; // Return -1 if the coordinates are invalid
    }
    board[y][x] = symbol; // Draw the point
    return 0;
}

// function to convert the board into a string to send to the clients
char *showBoard()
{
    int frame_width = BOARD_WIDTH + 2;
    int frame_height = BOARD_HEIGHT + 2;

    // Allocate memory for the string
    char *strboard = (char *)malloc((frame_width * frame_height + 1) * sizeof(char)); // Allocate memory
    if (strboard == NULL)
    {
    ...
    for (int i = 0; i < frame_height; i++)
    {
        for (int j = 0; j < frame_width; j++)
        {
            ...
            {
                strboard[index++] = '-'; // Top and bottom frame
            }
            else if (j == 0 || j == frame_width - 1)
            {
                strboard[index++] = '|'; // Left and right frame
            }
            else
            {
                strboard[index++] = board[i - 1][j - 1] == 0 ? ' ' : board[i - 1][j - 1]; // Board content
            }
        }
        strboard[index++] = '\n';
    }
...
// function to send the board to all clients
...
int main(int argc, char *argv[])
{
    ...
    int s_len;         // send length
    int r_len;         // receive length
    char buffer[1024]; // buffer for messages

    if (argc != 2)
    { // tikrinama ar portas nurodytas
        printf("USAGE: %s <port>\n", argv[0]);
        exit(1);
    }

    port = atoi(argv[1]); // konvertuojamas portas į skaičių

    if ((port < 1) || (port > 65535))
    { // tikrinama ar portas yra galiojantis
        printf("ERROR #1: invalid port specified.\n");
        exit(1);
    }

    struct pollfd pfds[MAX_CONNECTED_CLIENTS + 1];                  // pollfd array for client connections
    char usernames[MAX_CONNECTED_CLIENTS + 1][MAX_USERNAME_LENGTH]; // array for storing usernames
    memset(usernames, 0, sizeof(usernames));                        // initialize usernames array
    nfds_t nfds = MAX_CONNECTED_CLIENTS;                            // number of file descriptors
    int max_clients = MAX_CONNECTED_CLIENTS;                        // maximum number of clients
    ...
    pfds[0].fd = l_socket;   // server socket
    pfds[0].events = POLLIN; // poll for incoming connections

    for (int i = 1; i <= MAX_CONNECTED_CLIENTS; i++)
    {
        pfds[i].fd = -1; // initialize client sockets
    }
    ...
    { // infinite loop
        ...
        if (pfds[0].revents & POLLIN) // check if there is a new connection
        {
            ...
            {
                fprintf(stderr,
                        "ERROR #5: error occurred accepting connection.\n");
                exit(1);
            }

            for (int i = 1; i <= MAX_CONNECTED_CLIENTS; i++) // find an empty slot for the client
            {
                if (pfds[i].fd == -1)
                {
                    pfds[i].fd = c_socket;
        ...
        for (int i = 1; i <= MAX_CONNECTED_CLIENTS; i++) // check for incoming messages from clients
        {
            if (pfds[i].fd == -1) // skip empty slots
                continue;

            if (pfds[i].revents & POLLIN) // check if there is data to read
            {
                if (usernames[i][0] == '\0')
                {
                ...
                else // read messages from clients
                {
                    ...
                    else // process received messages
                    {
