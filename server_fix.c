 ...
 */
 ...
 char board[CANVAS_HEIGHT][CANVAS_WIDTH];
 ...
 int draw(int x, int y, char symbol)
 {
     ...
     board[y][x] = symbol; // Note the change here: board[y][x] for consistency
     return 0;
 }
 
 char* showBoard() {
     char *strboard = (char*)malloc((CANVAS_WIDTH * (CANVAS_HEIGHT + 1) + 1) * sizeof(char)); // Allocate memory
     if (strboard == NULL) {
 ...
 void sendBoardToClients(int current_client_fd, struct pollfd *pfds) {
     ...
     if (board_string != NULL) {
         char prefixed_board[CANVAS_WIDTH * (CANVAS_HEIGHT + 1) + 7]; // "BOARD:" + board + null
         sprintf(prefixed_board, "BOARD:%s", board_string);
         for (int i = 1; i <= MAX_CONNECTED_CLIENTS; i++) {
             if (pfds[i].fd != -1 && pfds[i].fd != current_client_fd) {
                 send(pfds[i].fd, prefixed_board, strlen(prefixed_board), 0);
             }
         }
 ...
 void commandParse (char *command, int client_fd, struct pollfd *pfds) {
     ...
     if (token == NULL) return; // Handle empty command
     ...
     if (strcmp(token, "/draw") == 0) {
                 ...
                 if (token != NULL) {
                     if (strlen(token) == 1) {
                         ...
                         if (check == -1) {
                             ...
                             sendBoardToClients(client_fd, pfds); // Send update to all clients
         ...
         sendBoardToClients(client_fd, pfds);
                 ...
                 if (pfds[i].revents & POLLIN)
                 {
                     if (usernames[i][0] == '\0')
                     {
                         ...
                         else
                         {
                             ...
                             // Send initial board to the newly connected client
