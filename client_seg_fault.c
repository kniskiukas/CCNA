...
// Structure to represent the local drawing board
typedef struct
{
...
// Update the local board based on server update
void update_local_board(char *board_string)
{
    int index = 0;
    for (int x = 0; x < CANVAS_WIDTH; x++)
    {
        for (int y = 0; y < CANVAS_HEIGHT; y++)
        {
            ...
            {
                board.grid[x][y] = board_string[index++];
            }
            else
            {
...
int main(int argc, char *argv[])
{
    ...
    bytes_received = recv(s_socket, buffer, BUFFER_SIZE - 1, 0); // Receive welcome message
    if (bytes_received > 0)
    {
    ...
    client_info_display(); // Display client info
    ...
    while (1)
    {
        ...
        FD_SET(s_socket, &readfds); // Add server socket
        ...
        int activity = select(s_socket + 1, &readfds, NULL, NULL, &tv); // Wait for activity
        ...
        {
            bytes_received = recv(s_socket, buffer, BUFFER_SIZE - 1, 0); // Receive data from server
            if (bytes_received > 0)
            {
                buffer[bytes_received] = '\0';
                ...
                {
                    update_local_board(buffer + 6); // Update board without "BOARD:" prefix
                    if (should_display)
                    {
                        client_info_display();
        ...
        {
            char ch;
            int read_size = read(0, &ch, 1); // Read a character from stdin
